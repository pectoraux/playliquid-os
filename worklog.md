...
Task ID: 13
Agent: orchestrator (main)
Task: Generic Artifact Execution Gate — declarative package format.

Work Log:
- Defined the declarative artifact format: LLMs produce structured JSON, not arbitrary JavaScript. Safe by construction, verifiable, portable.
- Built DeclarativePackageInterpreter: implements PackageInstance through the ABI, interprets declarative behaviors (patrol/wander/spin/static/pulse for update; circle/rect/diamond/triangle for render; emit/toggle/requestCapability for onClick).
- Built real certification (validateDeclarativeArtifact): structural validation that rejects invalid artifacts (valid: false). Checks required fields, valid behaviors, valid shapes, valid families.
- Built GenericArtifactLoader: replaces the hard-coded registry as the primary loader. resolveByName checks dev bootstrap (conformance tests only) first, then falls back to family default (declarative). resolveFromArtifact creates a PackageImplementation from any DeclarativeArtifact — no registry entry needed.
- Updated the import endpoint to use the real declarative certification (level: "verified", checks include "declarative-format-validated").
- Updated compile-prompt to ask the LLM for declarative JSON with the exact format specification.
- Added /api/packages/:id/artifact endpoint to return the declarative artifact for a package.
- Updated the browser runtime to use the generic loader.

The acceptance criterion: a new package generated by an external LLM (as declarative JSON) → imported → certified → RuntimeArtifact → loaded by the generic loader → executed by the PackageExecutor → rendered via engine-agnostic RenderContext — WITHOUT adding the package to any hard-coded registry.

Verified:
- compile-prompt produces a prompt asking for declarative JSON (3067 chars, contains "DECLARATIVE", "abiVersion", "patrol", "shape")
- Import validates the declarative artifact structurally (rejects invalid, accepts valid)
- The family fallback is now declarative — any package (including user-LLM imports) is executable
